...
export class QuestionService{
    ...
    async update(id:string,dto:UpdateQuestionDto):Promise<QuestionDto> {
      const foundModel = await this.readModelById(id);
      const upadatedmodel = QuestionMapper.mapUpdateQuestionToModel(dto,foundModel);
      try{
        const savedModel= await this.questionModelRepository.save(upadatedmodel);
        return QuestionMapper.mapToDto(savedModel);
      }catch (error){
        Logger.log(error,"QuestionService.upadte");
        throw  new BadRequestException();
      }
    }
